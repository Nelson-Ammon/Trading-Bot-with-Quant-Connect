class TrailingStopLoss(QCAlgorithm):
    ...
    def OnData(self, data):
        ...
        if (self.Time - self.stopMarketOrderFillTime).days < 20:
            return
        ...
        if not self.Portfolio.Invested and not self.Transactions.GetOpenOrders(self.spy):
            ...
            self.entryTicket = self.LimitOrder(self.spy, quantity, price, "Entry Order")
    ...
    def OnOrderEvent(self, orderEvent):
        ...
        if self.entryTicket is not None and self.entryTicket.OrderId == orderEvent.OrderId:
            ...
            self.stopMarketTicket = self.StopMarketOrder(self.spy, -self.entryTicket.Quantity, 0.80 * self.entryTicket.AverageFillPrice)
